...
An API client must first authenticate using OFBiz credentials to receive a token. This can be done in one of two ways:

* Supplying a Basic Authentication header that contains the username and password.
* Posting the username and password in the request body as JSON to the `/rest/auth/token/login` endpoint.
...
To authenticate without relying on Basic Authentication headers you can call the login endpoint directly with the credentials.
The REST component is mounted under `/rest` by default, so when using the standard embedded Tomcat configuration the fully-qualified
URL will be `https://localhost:8443/rest/auth/token/login`. Replace `localhost` and the port with the values that match your
deployment if you are running OFBiz behind a proxy or on a different host.

[source, json]
----
curl -X POST "https://localhost:8443/rest/auth/token/login" -H "Content-Type: application/json" -d '{"username":"admin","password":"ofbiz"}'
----

On success the response mirrors the structure returned by the Basic Authentication flow and contains both access and refresh tokens.
